...
local win = UILib:Window("KingScprit丨  乱杀中    自然灾害",Color3.fromRGB(102, 255, 153), Enum.KeyCode.RightControl)
...
tab:Toggle("灾难预测", false, function(bool)
...
local drop = tab4:Dropdown("玩家列表", plrlist, function(m)
...
local SkidFling = function(TargetPlayer)
    ...
    if Character and Humanoid and RootPart then
        ...
        if THumanoid and THumanoid.Sit and not AllBool then
            return Message("玩家消失", "tab4乱杀", 5) -- u can remove dis part if u want lol
        end
        if THead then
        ...
        elseif not TRootPart and not THead and Accessory and Handle then
            ...
            return Message("玩家已经退出/脚本承受不了", "认准tab4[霖溺]", 5)
        end
...
for _,x in next, Targets do
    if GetPlayer(x) and GetPlayer(x) ~= Player then
        if GetPlayer(x).UserId ~= 1414978355 then
            ...
            Message("检测到玩家消失", "tab4乱杀", 5)
        end
    elseif not GetPlayer(x) and not AllBool then
        Message("无敌中", "tab4乱杀", 5)
    end
end
end
end)

tab4:Button("甩飞所有", function()
local Targets = {"All"}
...
tab4:Toggle("循环甩飞",false, function(t)
if plr == nil then
 elseif plr ~= nil then
getgenv().autofling = t
spawn(function()
...
local SkidFling = function(TargetPlayer)
    ...
    if Character and Humanoid and RootPart then
        ...
        if THumanoid and THumanoid.Sit and not AllBool then
            return Message("Error❌", "tab4tab4", 5) -- u can remove dis part if u want lol
        end
        if THead then
            workspace.CurrentCamera.CameraSubject = THead
        elseif not THead and Handle then
...
tab4:Toggle("锁定传送",false, function(t)
if plr == nil then
 elseif plr ~= nil then
getgenv().autotele = t
spawn(function()
